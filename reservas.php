            ...
            function renderizarCalendario(ano, mes) {
                ...
                for (let dia = 1; dia <= totalDias; dia++) {
                    ...
                    reservas.forEach(r => {
                        ...
                        let dataComparacao = new Date(dataAtual);
                        dataComparacao.setHours(12, 0, 0, 0);

                        let diaInicio = new Date(r.data_checkin);
                        let diaFim = new Date(r.data_checkout);

                        // 🔴 DIA DE CHECK-IN
                        if (dataAtual.toDateString() === diaInicio.toDateString()) {
                            ocupado = true;
                            info = `<br><small>Entrada: ${r.hora_checkin}</small>`;
                        }

                        // 🔴 DIA DE CHECK-OUT
                        else if (dataAtual.toDateString() === diaFim.toDateString()) {
                            ocupado = true;
                            info = `<br><small>Saída: ${r.hora_checkout}</small>`;
                        }

                        // 🔴 DIAS INTERMEDIÁRIOS
                        else if (dataComparacao > inicio && dataComparacao < fim) {
                            ocupado = true;
